...
from unet import BBUNet, UNetEncoder, BBUNetDecoder
...
model_dir = join("model", "bb_unet", timestamp)
...
for outer_fold in kfold:
    ...
    for inner_fold in [0]:
        ...
        for epoch in epochs:
            ...
            for step, batch in enumerate(pbar):
                ...
                image, bbox_mask = image.to(device), bbox_mask.to(device)
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(pbar):            
            ...
            bbox_mask = batch["bbox_mask"]         
            target = batch["target"]
            image, bbox_mask, target = image.to(device), bbox_mask.to(device), target.to(device)
