...
# pretrained_checkpoint = "MedSAM/work_dir/LiteMedSAM/lite_medsam.pth"

timestamp = datetime.now()
timestamp = "%d_%d_%d_%02.0d_%02.0d_%02.0d" % (timestamp.year, timestamp.month, timestamp.day,
                                               timestamp.hour, timestamp.minute, timestamp.second)
model_dir = join("model", "litemedsam", timestamp)
...
config["pretrained_checkpoint"] = pretrained_checkpoint
...
for outer_fold in kfold:
    ...
    for inner_fold in [0]:
        ...
        if pretrained_checkpoint is not None and isfile(pretrained_checkpoint):
            ...
            print(f"Pretrained weights not found, training from scratch")
        ...
        for epoch in epochs:
            ...
            for step, batch in enumerate(pbar):
                ...
                image, target, bbox = image.to(device), target.to(device), bbox.to(device)
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(pbar):            
            ...
            target = batch["target"]
            bbox = batch["bbox"]
            image, bbox, target = image.to(device), bbox.to(device), target.to(device)
